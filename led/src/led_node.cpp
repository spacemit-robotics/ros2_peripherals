...
public:
  ...
  : rclcpp::Node("led_node")
  {
    try {
      ...
      const auto generic_sysfs_names = load_string_array(*this, "generic_sysfs_names", {""});
      const auto generic_active_levels = load_int_array(*this, "generic_active_levels", {0});

      const auto spi_dev_paths = load_string_array(*this, "spi_dev_paths", {"/dev/spidev2.0"});
      ...
      validate_config(
        led_ids, transports, names, generic_sysfs_names, generic_active_levels, spi_dev_paths,
        spi_num_leds, spi_speed_hz, spi_reset_bytes);

      state_pub_ = create_publisher<peripherals_led_node::msg::LedState>(
        state_topic_,
        rclcpp::QoS(rclcpp::KeepLast(std::max<size_t>(1, led_ids.size()))).reliable().transient_local());
      ...
      for (size_t i = 0; i < led_ids.size(); ++i) {
        register_led(
          ...
          static_cast<uint32_t>(spi_speed_hz[i]), static_cast<uint32_t>(spi_reset_bytes[i]));
      }
      ...
      RCLCPP_INFO(
        get_logger(),
        "led_node ready: command_topic=%s state_topic=%s leds=%zu tick_period_ms=%ld publish_period_ms=%ld",
        command_topic_.c_str(), state_topic_.c_str(), leds_.size(),
        static_cast<long>(tick_period_ms_), static_cast<long>(publish_period_ms_));
    } catch (...) {
      cleanup_resources();
      throw;
    }
...
private:
    ...
    const std::vector<int64_t> & spi_reset_bytes) const
  {
    ...
    const auto same_size = [led_count](size_t size, const char * field_name) {
        if (size != led_count) {
          throw std::invalid_argument(std::string(field_name) + " size mismatch with led_ids");
        }
      };
  ...
  void apply_command_locked(RegisteredLed & led, const peripherals_led_node::msg::LedCommand & cmd)
  {
    ...
    if ((cmd.mode == kLedModeBlink || cmd.mode == kLedModeBreath) && cmd.period_ms == 0) {
      RCLCPP_WARN(
        get_logger(), "period_ms must be > 0 for led_id=%u mode=%u", cmd.led_id, cmd.mode);
      return;
    }
  ...
  void mirror_tick_state(RegisteredLed & led, uint16_t dt_ms)
  {
    ...
    if (led.mode == kLedModeBreath) {
      ...
      uint32_t t = static_cast<uint32_t>(led.breath_elapsed_ms) + static_cast<uint32_t>(dt_ms);
      if (t >= led.period_ms) {
        t %= led.period_ms;
      }
      ...
      if (half == 0) {
        ...
        led.brightness = static_cast<uint8_t>((255U * (led.period_ms - t)) / half);
      }
      led.is_on = led.brightness > 0;
      return;
    }

    led.is_on = led.brightness > 0 && (led.red != 0 || led.green != 0 || led.blue != 0);
  }
