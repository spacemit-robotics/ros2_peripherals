...
{
  return state == WIFI_STA_CONNECTED ||
...
public:
  ...
  : rclcpp::Node("wifi_node")
  {
    ...
    RCLCPP_INFO(
      get_logger(),
      "wifi_node ready: status_topic=%s scan_topic=%s connect_service=%s disconnect_service=%s scan_service=%s status_period_ms=%d scan_period_ms=%d",
