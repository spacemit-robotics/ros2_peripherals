...
public:
        ...
        : rclcpp::Node("nfc_node")
    {
        ...
        tag_pub_ = create_publisher<peripherals_nfc_node::msg::NfcTagInfo>(
            tag_topic_, rclcpp::QoS(10).reliable());
        ...
        RCLCPP_INFO(
            get_logger(),
            "nfc_node ready: transport=%s driver=%s name=%s device=%s tag_topic=%s auto_poll=%s "
            "period_ms=%d timeout_ms=%d",
            transport_.c_str(), driver_.c_str(), instance_name_.c_str(), device_.c_str(),
            tag_topic_.c_str(), auto_poll_enabled_ ? "true" : "false", poll_period_ms_,
            poll_timeout_ms_);
    }
...
private:
    ...
    nfc_dev * alloc_device() const
    {
        ...
        if (transport_ == "i2c") {
            return nfc_alloc_i2c(
                full_name.c_str(), device_.c_str(), static_cast<uint8_t>(i2c_addr_));
        }
        if (transport_ == "spi") {
            return nfc_alloc_spi(
                full_name.c_str(), device_.c_str(), static_cast<uint32_t>(cs_pin_));
        }
        return nfc_alloc_uart(
            full_name.c_str(), device_.c_str(), static_cast<uint32_t>(baud_));
    }
        ...
        std::shared_ptr<peripherals_nfc_node::srv::NfcReadBlock::Response> resp)
    {
        ...
        if (rc > 0) {
            ...
            resp->message = "ok";
            resp->data.assign(
                buf.begin(), buf.begin() + std::min(static_cast<size_t>(rc), buf.size()));
            return;
        }
