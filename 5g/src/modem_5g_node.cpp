...
void set_service_result(ResponseT & response, enum modem_5g_status rc, const std::string & action)
{
  ...
  : rclcpp::Node("modem_5g_node")
  {
    try {
      ...
      flight_mode_srv_ = create_service<peripherals_5g_node::srv::Modem5gSetFlightMode>(
        ...
        std::bind(&Modem5gNode::on_set_flight_mode, this, std::placeholders::_1, std::placeholders::_2));
      ...
      set_pdp_srv_ = create_service<peripherals_5g_node::srv::Modem5gSetPdpContext>(
        ...
        std::bind(&Modem5gNode::on_set_pdp_context, this, std::placeholders::_1, std::placeholders::_2));
      get_pdp_srv_ = create_service<peripherals_5g_node::srv::Modem5gGetPdpContext>(
        get_pdp_service_name_,
        std::bind(&Modem5gNode::on_get_pdp_context, this, std::placeholders::_1, std::placeholders::_2));
...
private:
  ...
  void note_snapshot_error(
    peripherals_5g_node::msg::Modem5gStatus & msg, enum modem_5g_status rc, const std::string & action) const
  {
    if (msg.snapshot_ok) {
  ...
  {
    {
      ...
      set_service_result(*response, rc, "power_on");
    }
    publish_status_snapshot();
  }
  ...
  {
    {
      ...
      set_service_result(*response, rc, "power_off");
    }
    publish_status_snapshot();
  }
  ...
  {
    {
      ...
      set_service_result(*response, rc, "reset");
    }
    publish_status_snapshot();
  }
  ...
  {
    {
      ...
      set_service_result(*response, rc, "set_flight_mode");
      response->flight_mode_enabled = (rc == MODEM_5G_STATUS_SUCCESS) ? request->enable : flight_mode_enabled_;
    }
    publish_status_snapshot();
  }
    ...
    std::shared_ptr<peripherals_5g_node::srv::Modem5gSetPreferRat::Response> response)
  {
    ...
    if (rat == MODEM_5G_RAT_UNKNOWN) {
      set_service_result(*response, MODEM_5G_STATUS_INVALID, "set_prefer_rat");
      response->rat = 0;
      return;
    }

    {
      ...
      set_service_result(*response, rc, "set_prefer_rat");
    ...
    std::shared_ptr<peripherals_5g_node::srv::Modem5gSetPdpContext::Response> response)
  {
    ...
    if (cid < kMinCid || cid > kMaxCid) {
      set_service_result(*response, MODEM_5G_STATUS_INVALID, "set_pdp_context");
      return;
    }
...

    {
      ...
      set_service_result(*response, rc, "set_pdp_context");
    }
    publish_status_snapshot();
  }
    ...
    std::shared_ptr<peripherals_5g_node::srv::Modem5gGetPdpContext::Response> response)
  {
    const uint8_t cid = resolve_cid(request->cid);
    if (cid < kMinCid || cid > kMaxCid) {
      set_service_result(*response, MODEM_5G_STATUS_INVALID, "get_pdp_context");
      return;
    }
    ...
    struct modem_5g_pdp_context context {};
    {
      ...
      set_service_result(*response, rc, "get_pdp_context");
    }
    publish_status_snapshot();
  }
    ...
    std::shared_ptr<peripherals_5g_node::srv::Modem5gDataCall::Response> response)
  {
    const uint8_t cid = resolve_cid(request->cid);
    if (cid < kMinCid || cid > kMaxCid) {
      set_service_result(*response, MODEM_5G_STATUS_INVALID, "data_call");
      return;
    }
...

    {
      ...
      set_service_result(*response, rc, request->start ? "data_start" : "data_stop");
      response->cid = cid;
    ...
    std::shared_ptr<peripherals_5g_node::srv::Modem5gSendAt::Response> response)
  {
    if (request->command.empty()) {
      set_service_result(*response, MODEM_5G_STATUS_INVALID, "send_at");
      return;
    }
...

    {
      ...
      set_service_result(*response, rc, "send_at");
