    ...
    : rclcpp::Node("pm_node")
    {
        try {
            ...
            switch_service_name_ =
                declare_parameter<std::string>("switch_service", "power/set_switch");
            ...
            switch_srv_ = create_service<peripherals_pm_node::srv::SetPowerSwitch>(
                switch_service_name_,
                std::bind(
                    &PmNode::on_set_power_switch, this, std::placeholders::_1,
                    std::placeholders::_2));
...
private:
    ...
    {
        if (driver_ == "generic") {
            return pm_alloc_generic(
                instance_name_.c_str(), charger_node_.c_str(), capacity_node_.c_str(), nullptr);
        }
        if (driver_ == "adc") {
            return pm_alloc_adc(
                instance_name_.c_str(), device_.c_str(), static_cast<float>(adc_scale_));
        }
    ...
    void publish_status_snapshot()
    {
        ...
        if (rc != 0) {
            RCLCPP_WARN_THROTTLE(
                get_logger(), *get_clock(), 5000, "pm_get_state failed: %s",
        ...
        std::shared_ptr<peripherals_pm_node::srv::SetPowerSwitch::Response> resp)
    {
        ...
        if (rc != 0) {
            resp->success = false;
            resp->message =
                "pm_switch_set failed for channel " + std::to_string(channel_index) + " (" +
                channel_name + "): " + describe_rc(rc);
