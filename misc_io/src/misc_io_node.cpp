        ...
        try {
            ...
            for (size_t i = 0; i < io_ids.size(); ++i) {
                register_io(
                    ...
                    io_names.empty() ?
                    default_io_name(io_ids[i], chip_names[i], line_offsets[i]) :
                    io_names[i]);
            }
...
private:
        ...
        const std::string & consumer, const std::string & name)
    {
        ...
        RCLCPP_INFO(
            get_logger(),
            "registered io: io_id=%u type=%u dir=%u active_logic=%u debounce_ms=%u "
            "chip=%s line_offset=%u consumer=%s name=%s",
